...
# Part of step 2 and 6, a fuction I defined
def outlining(stars, outline, rows, height):
    #This takes a row of stars and hollows it out if outline is true.
    if (not outline) or (rows == height-1) or (rows == 0):
        return stars
    else:
        return stars[0] + stars[1:len(stars)-1].replace("*", " ") + stars[len(stars)-1]
...
def draw_pyramid(height, outline):
    ...
    for rows in range (height): 
        print((height - rows - 1)*" " + outlining(
            (2 * rows + 1)*"*", outline, rows, height))
...
def draw_circle(height, outline):
    if height == 1:
        ...
        for rows in range (height+1): 
            stars = (round(math.sqrt(
                (height / 2)**2 - (rows-height / 2)**2 ) + height / 2))*"*"
            print((height - len(stars))*" " + outlining(
                stars * 2 , outline, rows , height+1))
...
def draw_octagon(height, outline):
    for rows in range (height//3): #Top
        print((height//3 - rows)*" " + outlining(
            (2 * rows + height - 2*(height//3))*"*", outline, rows, height))
    ...
    for rows in range (height//3): #Bottom
        print((rows+1)*" " + outlining(
            (height - 2 * (rows+1))*"*", outline,
             rows + height - height//3, height))
